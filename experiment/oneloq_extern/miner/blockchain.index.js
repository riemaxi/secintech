...
function addBlockAndItem(prevBlock, item, handle){
	blockchainHash(prevBlock, item, (hash) => {
...
function doMining(powr, item, pattern, start, end, res){
	if (powr >= config.pow_rate){
		blockchain.mine(item, ()=>{
			new BlixScanner(sql, pattern, start, end).scan((blix, score) => res.json({ blix : blix, score : score }))
		})
	}else
